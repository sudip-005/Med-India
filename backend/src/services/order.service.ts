...
export class OrderService {
   ...
   */
  static async checkout(patientId: string, addressId: string, prescriptionId?: string) {
    // 1. Get cart items
    const { cart_id, items } = await CartService.getCartItems(patientId);
    ...
    if (itemsError) {
      ...
      await supabaseAdmin.from('orders').delete().eq('order_id', order.order_id);
   ...
   */
  static async getOrders(userId: string, role: 'PATIENT' | 'RETAILER' | 'DOCTOR') {
    if (role === 'PATIENT') {
      ...
      const { data: retailerItems, error: itemsError } = await supabaseAdmin
        .from('order_items')
        .select('order_id')
      ...
      return data.map((order) => ({
        ...order,
        items: order.items.filter((item: any) => item.retailer_id === userId),
      }));
    } else {
   ...
   */
  static async getOrderById(orderId: string, userId: string, role: 'PATIENT' | 'RETAILER' | 'DOCTOR') {
    ...
    if (role === 'RETAILER') {
      const hasSellerItem = order.items.some((item: any) => item.retailer_id === userId);
      if (!hasSellerItem) {
